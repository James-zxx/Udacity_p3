...
module.exports = function (ko) {
    function viewModel() {
        ...
        this.placeName = ko.observable('');//输入框值
        /**
         * @description 点击搜索进行位置搜索
         */
        this.searchMapName = function () {
            var sName = this.placeName;
            searchMapName(sName);
        };
    ...
    function initData() {
        ...
        for (var i = 0; i < this.mapData.length; i += 1) {
            ...
            location.name = this.mapData[i].name;
            location.position = this.mapData[i].center.split(',');
        ...
        $.ajax(
            {
                ...
                error: function (e) {
                    alert('服务器可能出小差了,暂时无法显示天气信息,亲爱的稍后再试');
                }
            }
        )
...
//监听输入框内容  并过滤
 ko.computed(function () {
        ...
        if (!viewModel.placeName()) {
            data_init.locations().forEach(function (lcoation) {
                lcoation.marker.show();//显示全部
            });
            viewModel.mapsList(data_init.locations());
        } else {
            var filterList = [];
            data_init.locations().filter(function (location) {
                // 输入框匹配条件
                var matched = location.name.toLowerCase().indexOf(viewModel.placeName().toLowerCase()) !== -1;
                if (matched) {
                    location.marker.show();
                    filterList.push(location);
                } else {
                    location.marker.hide();
                }

            });
            viewModel.mapsList(filterList);
        }
    });
